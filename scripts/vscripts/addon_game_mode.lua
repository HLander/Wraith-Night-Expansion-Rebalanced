...
function Precache( context )
	...
	PrecacheItemByNameSync( "item_saphire", context )
	PrecacheItemByNameSync( "item_slippers_of_halcyon", context )
	PrecacheItemByNameSync( "item_greater_clarity", context )
	PrecacheItemByNameSync( "item_dark_rapier", context )
	PrecacheItemByNameSync( "item_tpscroll", context )
	PrecacheItemByNameSync( "item_stormbearer", context )
	PrecacheItemByNameSync( "item_veil_of_mayaa", context )
	PrecacheItemByNameSync( "item_sphere_of_shin", context )
	PrecacheItemByNameSync( "item_saphire", context )
	PrecacheItemByNameSync( "item_tome_of_strength", context )
	PrecacheItemByNameSync( "item_tome_of_agility", context )
	PrecacheItemByNameSync( "item_tome_of_intellect", context )
	PrecacheItemByNameSync( "item_recipe_manta_datadriven", context )
...
--	GameRules:GetGameModeEntity():SetHUDVisible( DOTA_HUD_VISIBILITY_TOP_HEROES, false )
	GameRules:GetGameModeEntity():SetHUDVisible( DOTA_HUD_VISIBILITY_TOP_SCOREBOARD, true ) --***enable this
...
--	GameRules:GetGameModeEntity():SetHUDVisible( DOTA_HUD_VISIBILITY_INVENTORY_PROTECT, false )
	GameRules:GetGameModeEntity():SetHUDVisible( DOTA_HUD_VISIBILITY_INVENTORY_COURIER, true ) --***enable this
--	GameRules:GetGameModeEntity():SetHUDVisible( DOTA_HUD_VISIBILITY_INVENTORY_QUICKBUY, false )
	GameRules:GetGameModeEntity():SetHUDVisible( DOTA_HUD_VISIBILITY_SHOP_SUGGESTEDITEMS, true ) --***enable this
	GameRules:GetGameModeEntity():SetUseCustomHeroLevels( true ) 
	GameRules:GetGameModeEntity():SetCustomHeroMaxLevel( 40 )
	...
	ListenToGameEvent( 'player_disconnect', Dynamic_Wrap( CHoldoutGameMode, 'OnDisconnect' ), self )
	ListenToGameEvent( 'player_connect', Dynamic_Wrap( CHoldoutGameMode, 'PlayerConnect'), self )
	ListenToGameEvent( "player_reconnected", Dynamic_Wrap( CHoldoutGameMode, 'OnPlayerReconnect'), self )
	...
	for nPlayerID = 0, DOTA_MAX_TEAM_PLAYERS-1 do
		if PlayerResource:GetTeam( nPlayerID ) == DOTA_TEAM_GOODGUYS then
			if PlayerResource:HasSelectedHero( nPlayerID ) then
			    ...
			    --    DOTA_TEAM_GOODGUYS, Vector( 0, 0, 0 ), nil, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM_FRIENDLY,
			    --    DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_PLAYER_CONTROLLED, FIND_ANY_ORDER, false
			    --)
				if not hero:IsAlive() then
					--local respawnTimeFormula = hero:GetLevel() * 3.8 + 5
					--hero:SetRespawnPosition( hero:GetAbsOrigin() )
					--hero:SetTimeUntilRespawn(0)
					hero:RespawnUnit()
					--hero:SetTimeUntilRespawn(respawnTimeFormula)
				end
				--units:Heal(10000, ability)
...
function CHoldoutGameMode:_ResetInvul()
	...
	self._entAncient:SetInvulnCount( 4 )
end
...
ROUND_EXPECTED_VALUES_TABLE = {
	...
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 16
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 17
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 18
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 19
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 20
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 21
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 22
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 23
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 24
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 25
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 26
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 27
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 28
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] }, -- 29
	{ gold = 23368+STARTING_GOLD, xp = XP_PER_LEVEL_TABLE[18] } -- 30
}
...
function CHoldoutGameMode:OnDisconnect(keys)
...
function CHoldoutGameMode:OnPlayerReconnect(keys)
...
function CHoldoutGameMode:PlayerConnect(keys)
	print('PlayerConnect')
	DeepPrintTable(keys)
